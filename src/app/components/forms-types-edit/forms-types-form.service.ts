...
export class FormsTypesFormService {
  ...
  getForm(formsType: FormsTypes, hasVerbs: boolean): FormGroup {
    ...
    const in_language = new FormControl(formsType.in_language, [Validators.required, this.validateInLanguageIsUnic.bind(this)]);
    ...
    return new FormGroup({
      id, language_id, number, in_language, last_update, verbsIndicator
    }, { updateOn: 'blur' });
  }
  ...
  validateInLanguageIsUnic(control: AbstractControl) {
    ...
    if (!control.pristine && control.value !== '' && control.parent && control.parent.parent) {
      ...
      const listeMotsIdentiquesAuControle = formArray.value.filter(x => x.in_language === control.value);
